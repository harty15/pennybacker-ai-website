...
import { Figure, KeyTakeaways } from "@/components/mdx/figure";
import { HandoffVsOneTeam, DemoVsPilot } from "@/components/mdx/diagrams/gap";
...
<KeyTakeaways items={[
  "Pilots rarely fail on the model. They fail in the handoff between the people who wrote the plan and the people who built it.",
  "What falls into the gap is specific and boring: exceptions, field-level data quality, the baseline, the permission model, an owner for month four.",
  "A thicker requirements document does not fix it. One team from survey to production does.",
  "Build production-shaped from week one: real system of record, the user's own identity, an eval set, an audit trail.",
]} />
...
<Figure n={1} caption="The difference between the two models is the document hop. In the handoff model, whatever did not fit on the slides is gone by the time the build starts; in the one-team model the context travels in the people who write the code.">
  <HandoffVsOneTeam />
</Figure>
...
<Figure n={2} caption="Same box, different wiring. The demo runs on sample data under a service account; the pilot acts under the operator's identity against the real system of record, is graded by an evaluation set on every change, and logs every call from the first day.">
  <DemoVsPilot />
</Figure>
