...
import { ClassificationPipeline, TitleBlockAnatomy, RevisionChain } from "@/components/mdx/diagrams/documents";
...
<KeyTakeaways items={[
  "Document control stays the system of record; the AI proposes metadata, the register holds it.",
  "Every field is validated against a dictionary, a pattern, or the register itself, so the model never invents a discipline or a tag.",
  "Route by confidence: auto-accept above threshold with no rule failures, everything else to a review queue with the evidence beside the proposal.",
  "Revision tracking and cited answers fall out of complete metadata; reporting becomes a query.",
]} />
...
<Figure n={1} caption="Where each metadata value comes from on a real sheet: equipment tags and the revision cloud in the drawing area, and the document number, revision, status and discipline in the title block. Every value has a source of truth that is not the model.">
  <TitleBlockAnatomy />
</Figure>
...
<Figure n={2} caption="The pipeline. Classification produces a constrained record, validation attaches confidence and reasons, and routing decides whether document control sees a batch of proposals or a review item with evidence. Both paths write back to the register, which in turn supplies the dictionaries validation runs against.">
...
<Figure n={3} caption="Once revisions are linked, a superseded document is flagged everywhere it is cited, and the question that used to take a week becomes a diff with a citation.">
  <RevisionChain />
</Figure>
