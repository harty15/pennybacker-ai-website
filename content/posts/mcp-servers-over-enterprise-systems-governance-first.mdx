...
import { Figure, KeyTakeaways, Callout } from "@/components/mdx/figure";
import { McpTopology, CallLifecycle } from "@/components/mdx/diagrams/mcp";
...
<KeyTakeaways items={[
  "One MCP server per system of record, owned by the team that owns the system.",
  "Read first. Writes go through the approval workflow the operation already trusts.",
  "Identity passes through: the server acts as the user, and the system of record does the permission check.",
  "Typed, narrow tool inputs; every call logged; every answer cited; an eval set before the first demo.",
]} />
...
<Figure n={1} caption="The governance-first topology. The operator's identity travels with every call, each server wraps exactly one system and calls it as the user, the single write path goes through an approval gate, and every call from every server lands in one audit log.">
  <McpTopology />
</Figure>
...
<Figure n={2} caption="The life of one call. Arguments are validated against the tool schema and the user's access is checked in the system of record before anything is called; both refusals happen before a system is touched, and the answer carries the record identifiers the operator can open.">
  <CallLifecycle />
</Figure>

<Callout title="Prompts are not access control.">
  <p>If the only thing between a user and a record they should not see is an instruction in a system prompt, the record is exposed. Identity passthrough moves the check to the place that already enforces it.</p>
</Callout>
